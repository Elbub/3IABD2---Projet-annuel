...
if __name__ == "__main__":
    ...
    my_lib = ctypes.CDLL(r"..\_2023_3A_IABD1_Demo_Interop_Rust\target\debug\_2023_3A_IABD1_Demo_Interop_Rust.dll")
    ...
    my_lib.delete_float_array.argtypes = [ctypes.POINTER(ctypes.c_float), ctypes.c_int32]
    ...
    my_lib.predict_mlp_model.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float), ctypes.c_int32, ctypes.c_bool]
    my_lib.predict_mlp_model.restype = ctypes.POINTER(ctypes.c_float)

    all_training_inputs = np.array([
        [1.0, 8.0],
        [4.0, 2.0],
        [5.0, 6.0]
    ]).flatten()
    ...
    all_training_expected_outputs = np.array([
        [-1.0],
        [-1.0],
        [1.0]
    ]).flatten()
    ...
    my_lib.train_mlp_model.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_float), ctypes.c_int32, ctypes.c_int32,
                                       ctypes.POINTER(ctypes.c_float), ctypes.c_int32, ctypes.c_float, ctypes.c_int32,
                                       ctypes.c_bool]
    ...
    while True:
        ...
        native_predict_before_training = my_lib.predict_mlp_model(model_pointer, sample_inputs.ctypes.data_as(
            ctypes.POINTER(ctypes.c_float)), sample_inputs_size, True)

        print(f"Prediction before training : {np.ctypeslib.as_array(native_predict_before_training, (npl[-1],))}")
        my_lib.delete_float_array(native_predict_before_training, npl[-1])

        my_lib.train_mlp_model(model_pointer, all_training_inputs.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
                               all_training_inputs_size // npl[0], npl[0],
                               all_training_expected_outputs.ctypes.data_as(ctypes.POINTER(ctypes.c_float)), npl[-1],
                               0.1, 100, True)

        native_predict_after_training = my_lib.predict_mlp_model(model_pointer, sample_inputs.ctypes.data_as(
            ctypes.POINTER(ctypes.c_float)), sample_inputs_size, True)
        print(f"Prediction after training : {np.ctypeslib.as_array(native_predict_after_training, (npl[-1],))}")
